...
base.tidy = baseline.data.raw %>%
  mutate(responses = as.numeric(responses),
         board = factor(trial, levels=c("baseline_1","baseline_3","baseline_5"), labels=c("1/6 Board", "3/6 Board", "5/6 Board"))) %>%
  select(id, board, responses)
```
...
```{r}
# Histogram
ggplot(base.tidy, aes(x = responses, fill = board)) +
...
# Density plot
ggplot(base.tidy) +
  geom_density(aes(x = responses, fill = board), alpha=.2) +
  #facet_grid(~ board) +
  theme_minimal() +
  ggtitle("Histogram of baseline success ratings")

# Boxplot
ggplot(base.tidy, aes(x = board, y=responses,color=board)) +
...
Density plots for each dart board.
```{r}
